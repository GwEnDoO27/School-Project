...
func LogInHandle(w http.ResponseWriter, r *http.Request) error {
	...
	if r.FormValue("login-email") != "" && r.FormValue("login-psw") != "" {
		...
		email, err = GetEmailGithub(w, r)
		if err != nil {
			return err
		}
	} else if r.FormValue("FacebookUserContent") != "" {
		email, _, err = FaceBookGetData(r.FormValue("FacebookUserContent"))
		if err != nil {
			return err
		}
	}
	...
	if user.Username == "" || (bcrypt.CompareHashAndPassword([]byte(user.Password), []byte(password)) != nil && r.FormValue("userContent") == "") {
